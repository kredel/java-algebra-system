...
public class GroebnerBasePSTest extends TestCase {

    private static final Logger logger = Logger.getLogger(GroebnerBasePSTest.class);
 ...
 * @param name String
 */
   public GroebnerBasePSTest(String name) {
          super(name);
   }
 ...
 */ 
 public static Test suite() {
     TestSuite suite= new TestSuite(GroebnerBasePSTest.class);
     return suite;
   }
 ...
 */
 public void testGBase() {
     ...
     L = GroebnerBasePS.DIRPGB( L );
     assertTrue("isDIRPGB( { a } )", GroebnerBasePS.isDIRPGB(L) );
     ...
     L = GroebnerBasePS.DIRPGB( L );
     assertTrue("isDIRPGB( { a, b } )", GroebnerBasePS.isDIRPGB(L) );
     ...
     L = GroebnerBasePS.DIRPGB( L );
     assertTrue("isDIRPGB( { a, ,b, c } )", GroebnerBasePS.isDIRPGB(L) );
     ...
     L = GroebnerBasePS.DIRPGB( L );
     assertTrue("isDIRPGB( { a, ,b, c, d } )", GroebnerBasePS.isDIRPGB(L) );
     ...
     L = GroebnerBasePS.DIRPGB( L );
     assertTrue("isDIRPGB( { a, ,b, c, d, e } )", GroebnerBasePS.isDIRPGB(L) );
 }
 ...
 */
 public void testParallelGBase() {
     ...
     L = GroebnerBaseParallelPS.DIRPGB( L, threads );
     assertTrue("isDIRPGB( { a } )", GroebnerBasePS.isDIRPGB(L) );
 ...
 */
 public void xtestSequentialParallelGBase() {
     ...
     L.add(a);
     Gs = GroebnerBasePS.DIRPGB( L );
     Gp = GroebnerBaseParallelPS.DIRPGB( L, threads );
 ...
 */
 public void testSequentialDistributedGBase() {
	 ...
	 clients[t] = new Thread( new ClientsPS(host,port) );
	 clients[t].start();
     }
     try {
         Gp = GroebnerBaseDistributedPS.DIRPGBServer( L, threads, port );
     } catch (IOException e) {
     }
     for (int t = 0; t < threads; t++) {
	 try {
...
class ClientsPS implements Runnable {
    private final String host;
    private final int port;

    ClientsPS(String host, int port) {
	this.host = host;
	this.port = port;
    }
