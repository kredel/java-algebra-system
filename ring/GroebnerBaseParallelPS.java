...
public class GroebnerBaseParallelPS  {
    ...
    private static final Logger logger = Logger.getLogger(GroebnerBaseParallelPS.class);
	...
	for ( int i = 0; i < threads; i++ ) {
	      R = new ReducerPS( fin, P, pairlist );
	      T.addJob( R );
	}
	fin.done();
	P = DIGBMI(P,T);
    ...
    static class ReducerPS implements Runnable {
	private List P;
	private OrderedPairlistPS pairlist;
	private Terminator pool;
        private static Logger logger = Logger.getLogger(ReducerPS.class);

	ReducerPS(Terminator fin, List P, OrderedPairlistPS L) {
	    pool = fin;
	    this.P = P;
	    pairlist = L;
	} 
	...
	MiReducerPS[] mirs = new MiReducerPS[ P.size() ];
        int i = 0;
        Q = new ArrayList( P.size() );
        while ( P.size() > 0 ) {
            a = (OrderedPolynomial) P.remove(0);
	    // System.out.println("doing " + a.length());
	    mirs[i] = new MiReducerPS( (List)P.clone(), (List)Q.clone(), a );
	    T.addJob( mirs[i] );
	    i++;
            Q.add( (Object)a );
	}
    ...
    static class MiReducerPS implements Runnable {
	private List P;
	private List Q;
	private OrderedPolynomial S;
	private OrderedPolynomial H;
	private Semaphore done = new Semaphore(0);
        private static Logger logger = Logger.getLogger(MiReducerPS.class);

	MiReducerPS(List P, List Q, OrderedPolynomial p) {
	    this.P = P;
	    this.Q = Q;
	    S = p;
	    H = S;
