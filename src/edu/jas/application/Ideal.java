...
public class Ideal<C extends GcdRingElem<C>> implements Comparable<Ideal<C>>, Serializable, Cloneable {
     ...
     * @param h polynomial
     * @param Q quotient this : f^\infinity
    ...
    public IdealWithUniv<Quotient<C>> extension(QuotientRing<C> qfac) {
        ...
        GenPolynomialRing<GenPolynomial<C>> rfac = new GenPolynomialRing<GenPolynomial<C>>(efac,rvars.length,fac.tord,rvars);
     ...
     */
    public IdealWithUniv<C> permContraction(Ideal<Quotient<C>> eideal) {
     ...
     */
    public static <C extends GcdRingElem<C>> IdealWithUniv<C> contraction(Ideal<Quotient<C>> eideal) {
        ...
        List<GenPolynomial<C>> opols = new ArrayList<GenPolynomial<C>>(1);
        if ( f.isONE() ) {
            opols.add(f);
    ...
    public static <C extends GcdRingElem<C>> 
      IdealWithUniv<C> permutation(GenPolynomialRing<C> oring, IdealWithUniv<C> Cont) {
        ...
        List<GenPolynomial<C>> upolys = Cont.upolys;
        if ( upolys.size() > 0 ) {
            logger.error("upolys not empty = " + upolys);
            upolys = new ArrayList<GenPolynomial<C>>(0);
        }
    ...
    public List<IdealWithUniv<C>> radicalDecomposition() {
        ...
        for ( IdealWithUniv<Quotient<C>> ep : edec ) {
            IdealWithUniv<C> cont = permContraction(ep.ideal);
        ...
        SquarefreeAbstract<C> sqf = SquarefreeFactory.getImplementation(getRing().coFac);
        //not required: 
...
// 	for ( IdealWithUniv<C> od : dec ) {
//              System.out.println("od = " + od.ideal);
//              for ( IdealWithUniv<C> rd : Tdec ) {
//                  System.out.println("rd = " + rd.ideal);
// 		 Ideal<C> ir = od.ideal.intersect(rd.ideal);
...
// 	}
    ...
    public List<IdealWithUniv<C>> primeDecomposition() {
        ...
        List<Ideal<Quotient<C>>> qdec = new ArrayList<Ideal<Quotient<C>>>();
