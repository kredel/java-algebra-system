 ...
 * @author Heinz Kredel
 */
public class PolyUtilComp<C extends RingElem<C> > {

    private static final Logger logger = Logger.getLogger(PolyUtilComp.class);
    ...
    public static <C extends GcdRingElem<C>>
        ...
        plist = PolyUtilApp.<C>toProductRes( pfac, L );
        //System.out.println("\nplist ---------------------- ");
    ...
    public static <C extends RegularRingElem<C>>
        ...
        for ( int i = 0; i < s; i++ ) {
            p = D.list.remove(0);
            D = PolyUtilComp.<C>productDecomposition( D, p );
        }
            ...
            for ( Product<Residue<BigRational>> a : Pp.getMap().values() ) {
                ...
                for ( int i = 0; i < cfj /*pr.length()*/; i++ ) {
                    ...
                    if ( factoryInsert( pr, rrn ) ) {
                       ...
                       if ( pr.length() >= 0 ) {
                          Npp = PolyUtilComp.<Residue<BigRational>>productCoefficientExtension(Npp,j,pr.length()-1); 
                          changed = true;
                       }
                    }
                }
    ...
    public static <C extends RingElem<C>>
        ...
        for ( GenPolynomial<Product<C>> a : L ) {
            GenPolynomial<Product<C>> b 
                = PolyUtilComp.<C>productCoefficientExtension(a,i,j);
            if ( ! b.isZERO() ) {
               M.add( b );
            }
        }
