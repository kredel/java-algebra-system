 ...
 * Select appropriate squarefree factorization engine based on the coefficient types.
 * @author Heinz Kredel
 * @usage To create objects that implement the <code>Squarefree</code> interface use the
 * <code>SquarefreeFactory</code>. It will select an appropriate 
 ...
 * @see edu.jas.ufd.SquarefreeAbstract#squarefreeFactors(edu.jas.poly.GenPolynomial P)
 */
...
public class SquarefreeFactory {
    ...
    public static <C extends GcdRingElem<C>> 
        ...
        if ( t == 0 ) {
            throw new RuntimeException("no squarefree factorization implementation for " + fac.getClass().getName());
        }
        if ( t == 1 ) { // BigInteger
