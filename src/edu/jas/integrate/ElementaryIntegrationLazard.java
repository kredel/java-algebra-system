...
public class ElementaryIntegrationLazard<C extends GcdRingElem<C>> extends ElementaryIntegration<C> {
	...
	/**
	 * Univariate GenPolynomial integration of the logaritmic part, Lazard - Rioboo
	 * - Trager
	 * 
	 * @param A
	 *            univariate GenPolynomial, deg(A) < deg(P).
	 * @param P
	...
	public LogIntegral<C> integrateLogPart(GenPolynomial<C> A, GenPolynomial<C> P) {
		...
		List<GenPolynomial<GenPolynomial<C>>> RcList = engine.recursiveUnivariateSubResultantCoefficients(Pc, At); // returning
																													// the
		...
		for (Entry<GenPolynomial<C>, Long> qi : resSquareFree.entrySet()) {
			...
			// K(alpha)[x]
			GenPolynomialRing<AlgebraicNumber<C>> pafac = new GenPolynomialRing<AlgebraicNumber<C>>(afac, Pc.ring);
			...
			if (qi2.degree() > 0) {
				if (qi.getValue() == Pc.degree()) {
					...
					for (GenPolynomial<C> ai : sfactors.keySet()) {
						GenPolynomial<AlgebraicNumber<C>> aiC = PolyUtil.<C>convertToAlgebraicCoefficients(pafac, ai);
						GenPolynomial<AlgebraicNumber<C>> qiC = PolyUtil.<C>convertToAlgebraicCoefficients(pafac, qi2);
						if (aengine == null) {
